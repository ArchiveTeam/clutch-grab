...
allowed = function(url, parenturl, force)
  ...
  if string.match(url, "'+")
    or string.match(url, "[<>\\%*%$;%^%[%],%(%){}\n]")
    or string.match(url, "^https?://[^/]*fastly%.net/media/avatars/")
    or string.match(url, "^https?://[^/]*fastly%.net/media/games/") then
    return false
  end
  ...
  if string.match(url, "^https?://[^/]*fastly%.net/")
    ...
    if parenturl and string.match(parenturl, "^https?://api%.clutch%.win/") then
      ids[string.match(url, "^https?://[^/]+(/.+)$")] = true
      return true
    end
    return false
  end
